...
class Unet(nn.Module):
    ...
    def forward(self, u, grid, dt, variables, time=None):
        x = u
        coeffs = variables
        x = self.preprocess(x, grid, dt, coeffs)
        x = self.init_conv(x)
    ...
    def __repr__(self): 
        return f"Unet({self.channels}, {self.eq_variables})"
    
    def preprocess(self, u, grid, dt, variables):
