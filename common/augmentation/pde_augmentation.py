...
def fourier_shift(u: torch.Tensor, eps: float=0., dim: int=-1, order: int=0) -> torch.Tensor:
    ...
    # Fourier modes
    omega = torch.arange(n // 2 + 1).to(u.device)
    if n % 2 == 0:
        omega[-1] *= 0
...
class LinearShift:
    ...
    def __call__(self, u, dim=-1):
        n = u.shape[dim]
        # Shift to the left and to the right and interpolate linearly
        q, r = torch.div(self.shift*n, 1, rounding_mode='floor'), (self.shift * n) % 1
        q_left, q_right = q/n, (q+1)/n
        u_left = fourier_shift(u, eps=q_left, dim=-1)
        u_right = fourier_shift(u, eps=q_right, dim=-1)
        return (1-r) * u_left + r * u_right
...
class Galileo:
    ...
    def __call__(self, sample: tuple, eps: Optional[float]=None, shift: str='fourier') -> torch.Tensor:
        """
        Galilean shift.
        Args:
            sample (torch.Tensor): input tensor of the form [u, X]
        ...
        dx = X[0, 1, 0] - X[0, 0, 0]
        dt = X[1, 0, 1] - X[0, 0, 1]
        t = dt * torch.arange(T).to(u.device)
        L = dx * N
...
class KdV_augmentation:
    def __init__(self, max_x_shift: float = 1.0, max_velocity: float = 1.0, max_scale = 1.0):
        """
...
class Heat_augmentation:
    def __init__(self, max_x_shift: float = 1.0):
        """
